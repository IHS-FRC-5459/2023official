...
public class DriveCourseF extends CommandBase {
  /** Creates a new DriveCourseF. */
  private double pwr, course, distanceInInches;
  public DriveCourseF(double pwr, double course, double distanceInInches) {
    this.pwr = pwr;
    this.course=course;
  ...
  public void execute() {
    ...
    if(currentAng > course){
      ...
      distance = 1 - distance;
      Robot.m_robotContainer.m_DriveSub.setDrive(pwr * distance, pwr);
    } else if(currentAng < course){
      double distanceO = course - currentAng;
      distanceO = (distanceO / 100 ) * 3;
      distanceO = 1 - distanceO;
      Robot.m_robotContainer.m_DriveSub.setDrive(pwr , pwr* distanceO);
    }else{
      Robot.m_robotContainer.m_DriveSub.setDrive(pwr, pwr);
    }
  }
